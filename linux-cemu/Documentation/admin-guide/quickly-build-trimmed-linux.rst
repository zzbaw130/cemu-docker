...
you then use to configure, build and install your own kernel::
    ...
    command -v installkernel && make modules_install install
    reboot
 ...
 * Your kernel is easy to remove later, as its parts are only stored in two
   ...
   after its release name -- '6.0.1-foobar' in the following example::
     ...
     rm -rf /lib/modules/6.0.1-foobar
   ...
   bootloader configuration::
     ...
     command -v kernel-install && kernel-install -v remove 6.0.1-foobar
 ...
 * Debian, Ubuntu, and derivatives::
     ...
     apt install bc binutils bison dwarves flex gcc git make openssl \
       pahole perl-base libssl-dev libelf-dev

 * Fedora and derivatives::

     dnf install binutils /usr/include/{libelf.h,openssl/pkcs7.h} \
       /usr/bin/{bc,bison,flex,gcc,git,openssl,make,perl,pahole}

 * openSUSE and derivatives::

     zypper install bc binutils bison dwarves flex gcc git make perl-base \
...
build system and then install the image and the System.map file manually::
     ...
     install -m 0600 $(make -s image_name) /boot/vmlinuz-$(make -s kernelrelease)
     install -m 0600 System.map /boot/System.map-$(make -s kernelrelease)
