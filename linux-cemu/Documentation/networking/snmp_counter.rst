...
Change tcp_max_orphans to a smaller value on client::
  ...
  bash -c "echo 10 > /proc/sys/net/ipv4/tcp_max_orphans"
...
Run iptables on server::
  ...
  iptables -A INPUT -i ens3 -p tcp --destination-port 9000 -j DROP
...
Prepare on server A, add a route to server B::
  ...
  $ ip route add 8.8.8.8/32 via 192.168.122.251

Prepare on server B, disable send_redirects for all interfaces::

  $ sysctl -w net.ipv4.conf.all.send_redirects=0
  $ sysctl -w net.ipv4.conf.ens3.send_redirects=0
...
Check the route table of server B and remove the default route::
  ...
  $ ip route delete default via 192.168.122.1 dev ens3 proto static
...
Run tcpdump to capture a SYN packet::
  ...
  nstatuser@nstat-a:~$ tcpdump -c 1 -w /tmp/syn.pcap port 9000
...
RST to nstat-b::
  ...
  nstatuser@nstat-a:~$ iptables -A INPUT -p tcp --sport 9000 -j DROP

Send 3 SYN repeatedly to nstat-b::

  nstatuser@nstat-a:~$ for i in {1..3}; do tcpreplay -i ens3 /tmp/syn_fixcsum.pcap; done
...
On nstat-a, run tcpdump to capture a SYN::
  ...
  nstatuser@nstat-a:~$ tcpdump -w /tmp/paws_pre.pcap -c 1 port 9000
  tcpdump: listening on ens3, link-type EN10MB (Ethernet), capture size 262144 bytes
...
Send the SYN packet twice::
  ...
  nstatuser@nstat-a:~$ for i in {1..2}; do tcpreplay -i ens3 /tmp/paws.pcap; done
...
On nstat-a, run tcpdump to capture an ACK::
  ...
  nstatuser@nstat-a:~$ tcpdump -w /tmp/seq_pre.pcap -c 1 dst port 9001
  tcpdump: listening on ens3, link-type EN10MB (Ethernet), capture size 262144 bytes
...
Now the /tmp/seq.pcap is the packet we need. Send it to nstat-b::
  ...
  nstatuser@nstat-a:~$ for i in {1..2}; do tcpreplay -i ens3 /tmp/seq.pcap; done
