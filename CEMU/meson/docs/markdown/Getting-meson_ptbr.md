...
`pip3 install meson` que requer root e vai instala-lo para todo o sistema.

Se você já baixou uma cópia do código do Meson, você pode instalar com
`pip3 install path/to/source/root/`.
...
para o seu `PATH`, e `meson install` vai estar completamente quebrado já que
