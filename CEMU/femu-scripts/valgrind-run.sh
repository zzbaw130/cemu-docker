...
if [[ $is_mounted == "" ]]; then
    mkdir -p /mnt/tmpfs
    # huge=always
    mount -t tmpfs -o size=128G,huge=always tmpfs /mnt/tmpfs
fi


# every time we run a new SSD
rm -rf /mnt/tmpfs/test1.raw

[[ ! -e /mnt/tmpfs/test1.raw ]] && ./qemu-img create -f raw /mnt/tmpfs/test1.raw 128G

# huge page related settings
#echo 25000 | tee /proc/sys/vm/nr_hugepages

[[ ! -d /dev/hugepages2M ]] && mkdir /dev/hugepages2M && mount -t hugetlbfs none /dev/hugepages2M -o pagesize=2M
...
valgrind x86_64-softmmu/qemu-system-x86_64 \
    -name "nvme-FEMU-test" \
    -enable-kvm \
    -cpu host \
    -smp 16 \
...
#./pin.sh
#sshsim "~/tsc.sh"
#sshsim "echo 0 | tee /proc/sys/kernel/timer_migration"
#sshsim "echo 0 | tee /sys/kernel/debug/tracing/tracing_on"
