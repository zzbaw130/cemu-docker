                               ...
                               NvmeRequest *req)
{
    ...
    if (opcode == OC12_CMD_ERASE) {
        ...
        req->stat.expire_time = advance_chip_timestamp(n, lunid, now, opcode, 0);
        return 0;
    }
...
static uint16_t oc12_admin_cmd(NvmeNamespace *ns, NvmeCmd *cmd, NvmeRequest *req)
{
    FemuCtrl *n = ns->ctrl;
    switch (cmd->opcode) {
...
static uint16_t oc12_io_cmd(NvmeNamespace *ns, NvmeCmd *cmd, NvmeRequest *req)
{
    FemuCtrl *n = ns->ctrl;
    switch (cmd->opcode) {
    case NVME_CMD_READ:
...
{
    static atomic_int fsid_voc12 = ATOMIC_VAR_INIT(0);
    const char *vocssd12_mn = "FEMU OpenChannel-SSD Controller (v1.2)";
    const char *vocssd12_sn   = "vOCSSD";
    int id = atomic_fetch_add(&fsid_voc12, 1);

    nvme_set_ctrl_name(n, vocssd12_mn, vocssd12_sn, &id);
}
...
int nvme_register_ocssd12(NvmeNamespace *ns)
{
