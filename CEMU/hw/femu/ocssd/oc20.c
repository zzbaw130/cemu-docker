                               ...
                               NvmeRequest *req)
{
    ...
    if (opcode == OC20_CMD_VECT_ERASE) {
        ...
        for (i = 0; i < nlb; i++) {
            ...
            if (ts > req->stat.expire_time) {
                req->stat.expire_time = ts;
            }
        }
    ...
    req->stat.expire_time = now + total_time_need_to_emulate;
...
static uint16_t oc20_rw(FemuCtrl *n, NvmeCmd *cmd, NvmeRequest *req, bool vector)
{
    ...
    backend_rw(ns->backend, &req->qsg, aio_sector_list, req->is_write);
...
static uint16_t oc20_admin_cmd(NvmeNamespace *ns, NvmeCmd *cmd, NvmeRequest *req)
{
    FemuCtrl *n = ns->ctrl;
    switch (cmd->opcode) {
...
static uint16_t oc20_io_cmd(NvmeNamespace *ns, NvmeCmd *cmd, NvmeRequest *req)
{
    FemuCtrl *n = ns->ctrl;
    switch (cmd->opcode) {
    case NVME_CMD_READ:
...
{
    Oc20Ctrl *ln = n->cmd_set_ops[NVME_CSI_ZONED].state;
...
{
    static atomic_int fsid_voc20 = ATOMIC_VAR_INIT(0);
    const char *vocssd20_mn = "FEMU OpenChannel-SSD Controller (v2.0)";
    const char *vocssd20_sn = "vOCSSD";
    int id = atomic_fetch_add(&fsid_voc20, 1);

    nvme_set_ctrl_name(n, vocssd20_mn, vocssd20_sn, &id);
}
...
static void oc20_init(NvmeNamespace *ns, Error **errp)
{
...
int nvme_register_ocssd20(NvmeNamespace *ns)
{
    Oc20Ctrl *ln = g_malloc0(sizeof(Oc20Ctrl));
    ns->csi = NVME_CSI_ZONED;
    ns->ops = (CommandSetOps) {
        .state            = ln,
        .init             = oc20_init,
        .exit             = oc20_exit,
