...
static void nvme_init_poller(FemuCtrl *n)
{
    ...
    if (n->cq_poller_enabled)
        n->cq_poller = g_malloc0(sizeof(QemuThread) * (n->nr_pollers + 1));
    ...
    for (i = 1; i <= n->nr_pollers; i++) {
        ...
        if (n->cq_poller_enabled) {
            qemu_thread_create(&n->cq_poller[i], "femu-nvme-cq-poller",
                    nvme_cq_poller, &args[i], QEMU_THREAD_JOINABLE);
            femu_debug("femu-nvme-cq-poller [%d] created ...\n", i - 1);
        }
    }
}
...
static uint16_t nvme_identify_ns_csi(FemuCtrl *n, NvmeCmd *cmd)
{
    ...
    if (c->csi == NVME_CSI_NVM && nvme_csi_has_nvm_support(ns)) {
        return nvme_rpt_empty_id_struct(n, cmd);
    } else if (c->csi == NVME_CSI_MEMORY && ns->csi == NVME_CSI_MEMORY) {
        return nvme_rpt_empty_id_struct(n, cmd);
...
static uint16_t nvme_identify_ctrl_csi(FemuCtrl *n, NvmeCmd *cmd)
{
    ...
    if (c->csi == NVME_CSI_NVM) {
        return nvme_rpt_empty_id_struct(n, cmd);
    } else if (c->csi == NVME_CSI_MEMORY) {
        return nvme_rpt_empty_id_struct(n, cmd);
    } else if (c->csi == NVME_CSI_COMPUTE) {
        return nvme_rpt_empty_id_struct(n, cmd);
...
static uint16_t nvme_identify_ns_descr_list(FemuCtrl *n, NvmeCmd *cmd)
{
    ...
    memcpy(&ns_descrs->uuid.v, ns->uuid.data, NVME_NIDL_UUID);
...
static uint16_t nvme_identify_cmd_set(FemuCtrl *n, NvmeCmd *cmd)
{
    ...
    NVME_SET_CSI(*list, NVME_CSI_MEMORY);
...
static uint16_t nvme_identify_ns_cs_indep(FemuCtrl *n, NvmeCmd *cmd)
{
    ...
    return dma_read_prp(n, (uint8_t *)&ns->id_ns_cs_indep,
                        sizeof(NvmeIdNsCsIndep), prp1, prp2);
}
...
static uint16_t nvme_identify(FemuCtrl *n, NvmeCmd *cmd)
{
    ...
    femu_debug("nvme_identify: cns=%d\n", cns);
...
static uint16_t nvme_admin_cmd(FemuCtrl *n, NvmeCmd *cmd, NvmeCqe *cqe)
{
    ...
    default:
        if (ns->ops->admin_cmd) {
            return ns->ops->admin_cmd(ns, cmd, cqe);
        }
