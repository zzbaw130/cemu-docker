                             ...
                             bool wrz)
{
    ...
    if (!wrz) {
        ...
        backend_rw(ns->backend, &req->qsg, &data_offset, req->is_write);
    }
...
static uint16_t zns_admin_cmd(NvmeNamespace *ns, NvmeCmd *cmd, NvmeCqe *cqe)
{
    switch (cmd->opcode) {
    default:
        return NVME_INVALID_OPCODE | NVME_DNR;
                         ...
                         NvmeRequest *req)
{
    ...
    for (lpn = slpn; lpn <= elpn; lpn++) {
        ...
        read.cmd = NAND_READ;
        read.stime = req->stat.stime;

        sublat = zns_advance_status(n, &read, &ppa);
        maxlat = (sublat > maxlat) ? sublat : maxlat;
    }

    req->stat.reqlat += maxlat;
    req->stat.expire_time += maxlat;
...
static int zns_io_cmd(NvmeNamespace *ns, NvmeCmd *cmd, NvmeRequest *req)
{
    FemuCtrl *n = ns->ctrl;
    // TODO: zns don't have ftl thread?
    switch (cmd->opcode) {
    case NVME_CMD_READ:
        req->status = zns_read(n, ns, cmd, req);
        return 1;
    case NVME_CMD_WRITE:
        req->status = zns_write(n, ns, cmd, req);
        return 1;
    case NVME_CMD_ZONE_MGMT_SEND:
        req->status = zns_zone_mgmt_send(n, req);
        return 1;
    case NVME_CMD_ZONE_MGMT_RECV:
        req->status = zns_zone_mgmt_recv(n, req);
        return 1;
    case NVME_CMD_ZONE_APPEND:
        req->status = zns_zone_append(n, req);
        return 1;
    }
...
{
    static atomic_int fsid_zns = ATOMIC_VAR_INIT(0);
    const char *zns_mn = "FEMU ZNS-SSD Controller";
    const char *zns_sn = "vZNSSD";
    int id = atomic_fetch_add(&fsid_zns, 1);

    nvme_set_ctrl_name(n, zns_mn, zns_sn, &id);
}
...
static void zns_init(NvmeNamespace *ns, Error **errp)
{
    zns_set_ctrl(ns->ctrl);

    zns_init_zone_cap(ns->ctrl);

    if (zns_init_zone_geometry(ns, errp) != 0) {
        return;
    }

    zns_init_zone_identify(ns->ctrl, ns, 0);
    zns_init_params(ns->ctrl);
}

static void zns_exit(NvmeNamespace *n)
{
    /*
...
CommandSetOps zone_command_set_ops = {
    .state            = NULL,
    .init             = zns_init,
    .exit             = zns_exit,
    .rw_check_req     = NULL,
...
nvme_register(NVME_CSI_ZONED, &zone_command_set_ops);