...
// nvme command set identifier
enum NvmeCsi {
    NVME_CSI_NVM     = 0x00,
    // NVME_CSI_KV      = 0x01,
    NVME_CSI_ZONED   = 0x02,
    NVME_CSI_MEMORY  = 0x03,
    NVME_CSI_COMPUTE = 0x04,
    NVME_CSI_MAX,
};

static inline const char *nvme_csi_str(uint8_t csi)
{
    switch (csi) {
    case NVME_CSI_NVM:
        return "nvm";
...
static inline uint8_t nvme_csi_from_str(const char *str)
{
    if (!strcmp(str, "nvm")) {
        return NVME_CSI_NVM;
    } else if (!strcmp(str, "zoned")) {
        return NVME_CSI_ZONED;
    } else if (!strcmp(str, "memory")) {
        return NVME_CSI_MEMORY;
    } else if (!strcmp(str, "compute")) {
        return NVME_CSI_COMPUTE;
    } else {
        return NVME_CSI_MAX;
    }
}
...
typedef struct NvmeIdNsCsIndep {
    uint8_t     nsfeat;
    uint8_t     nmic;
    uint8_t     rescap;
    uint8_t     fpi;
...
typedef struct NvmeRequest {
    ...
    /* CSD */
    struct ComputeJob       *job;
    /* indirect usage model */
    QemuThread              *indirect_thread;
    NvmeCopyFormat          *sres;
    int                     nr_sres;
    int                     p2p_req;
...
enum NvmeNsImpl {
    ...
    NVME_NS_IMPL_OC20,
    NVME_NS_IMPL_MEMORY,
    NVME_NS_IMPL_COMPUTE,
};

typedef struct CommandSetOps {
    void     *state;
    void     (*init)(NvmeNamespace *, Error **);
    void     (*exit)(NvmeNamespace *);
    uint16_t (*rw_check_req)(NvmeNamespace *, NvmeCmd *, NvmeRequest *);
    int      (*start_ctrl)(FemuCtrl *);
    uint16_t (*admin_cmd)(NvmeNamespace *, NvmeCmd *, NvmeCqe *);
    int      (*io_cmd)(NvmeNamespace *, NvmeCmd *, NvmeRequest *);  // return 1 when the command is finished after function call
    uint16_t (*get_log)(NvmeNamespace *, NvmeCmd *);
    int      (*decode_params)(const cJSON *, NvmeNamespace *);  // parse json
    int      (*encode_params)(cJSON *, NvmeNamespace *);        // dump json
} CommandSetOps;

extern CommandSetOps *command_set_ops[NVME_CSI_MAX];

#define nvme_register(csi, ops)                                         \
static void __attribute__((constructor)) nvme_register_##csi(void) {    \
    command_set_ops[csi] = ops;                                         \
}

typedef struct CsfSchedulerOption {
    bool take_care_of_group;
...
extern pthread_spinlock_t csd_memory_region_lock;
extern int nr_csd_memory_region;
extern struct CsdMemoryRegion csd_memory_region[64];

typedef struct NvmeNamespace {
    FemuCtrl            *ctrl;
    SsdBackend          *backend;
    void                *params;    // namespace parameters
    void                *private;   // namespace private data
    NvmeIdNs            id_ns;
    NvmeIdNsCsIndep     id_ns_cs_indep;
    NvmeRangeType       lba_range[64];
    enum NvmeCsi        csi;
    enum NvmeNsImpl     impl;
    CommandSetOps       *ops;
    QemuUUID            uuid;
    unsigned long       *util;
...
typedef struct FemuCtrl {
    ...
    /* Coperd: OC2.0 FIXME */
    NvmeParams  params;
    CommandSetOps cmd_set_ops[NVME_CSI_MAX];
    ...
    /* PCIe */
    uint8_t  pcie_bandwidth;            // GB
    uint32_t pcie_propagation_delay;    // ns
    uint64_t pcie_tx_next_avail_time;
    uint64_t pcie_rx_next_avail_time;

    /* Internal Bandwidth */
    uint64_t internal_bandwidth;        // MB, 0 means no limit
    ...
    int             drive_id;   // multiple drive support
    int             stat_shm_fd;
    FemuStat        *stat;
    char            devname[64];
    ...
    // used to calculate memcpy bandwidth. need FEMU_DEBUG_MEMCPY
...
static inline NvmeNamespace *nvme_find_namespace(FemuCtrl *n, uint32_t nsid)
{
...
// used to calculate memcpy bandwidth
// #define FEMU_DEBUG_MEMCPY
