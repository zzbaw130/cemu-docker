...
CommandSetOps *command_set_ops[NVME_CSI_MAX];

extern pthread_spinlock_t stat_lock;
pthread_spinlock_t csd_memory_region_lock;
int totoal_drive = 0;
int nr_csd_memory_region = 0;
struct CsdMemoryRegion csd_memory_region[64];
...
static void nvme_clear_ctrl(FemuCtrl *n, bool shutdown)
{
    ...
    if (shutdown) {
        femu_log("nr_tt_ios: %lu, nr_tt_late_ios: %lu, %f%%\n", n->nr_tt_ios, n->nr_tt_late_ios, n->nr_tt_late_ios * 100.0 / n->nr_tt_ios);
        femu_log("max_processed: %lu\n", n->stat->max_processed);
        int64_t stop = clock_ns();
        femu_log("memcpy time: %.2f%%\n", n->memcpy_time * 100.0 / (stop - n->start_time));
        femu_log("memcpy bw: %.2fMB/s\n", n->memcpy_size * 1000.0 / n->memcpy_time);
        femu_debug("femu_stat_exit()\n");
        femu_stat_exit(n);
    }
...
{
    if (n->num_namespaces == 3) {
        pthread_spin_lock(&csd_memory_region_lock);
        csd_memory_region[nr_csd_memory_region].dma_addr = n->slm_mem.addr;
        csd_memory_region[nr_csd_memory_region].backend_addr = (uintptr_t)n->namespaces[1].backend->logical_space;
        csd_memory_region[nr_csd_memory_region].size = n->namespaces[1].backend->size;
        csd_memory_region[nr_csd_memory_region].n = n;
        pthread_spin_unlock(&csd_memory_region_lock);
        femu_log("CEMU BAR 2 addr: 0x%lx, size: 0x%lx, backend addr: 0x%lx, nr_csd_memory_region: %d, drive_id: %d\n",
            n->slm_mem.addr, n->namespaces[1].backend->size,
            csd_memory_region[nr_csd_memory_region].backend_addr, nr_csd_memory_region+1, n->drive_id);
        nr_csd_memory_region++;
    }
    ...
    if (n->femu_mode == FEMU_ZNSSD_MODE) {
        n->cmd_set_ops[NVME_CSI_ZONED].start_ctrl(n);
    }
...
{
    FemuCtrl *n = (FemuCtrl *)opaque;
    // uint64_t bar_addr = pci_get_bar_addr(&n->parent_obj, 2);
    // femu_log("BAR 2 addr: %lx\n", bar_addr);
    if (addr < sizeof(n->bar)) {
...
static uint64_t slm_mmio_read(void *opaque, hwaddr addr, unsigned size)
{
    // FemuCtrl *n = (FemuCtrl *)opaque;
    // uint64_t bar_addr = pci_get_bar_addr(&n->parent_obj, 2);
    uint64_t val = 128;

    // femu_debug("slm_mmio_read, addr: %lu, size: %u, BAR 2 addr %lx\n", addr, size, bar_addr);

    return val;
}

static void slm_mmio_write(void *opaque, hwaddr addr, uint64_t data, unsigned size)
{
    // FemuCtrl *n = (FemuCtrl *)opaque;
    // uint64_t bar_addr = pci_get_bar_addr(&n->parent_obj, 2);
    // femu_debug("slm_mmio_write, addr %lu, size %u, BAR 2 addr %lx\n", addr, size, bar_addr);
}
...
static const MemoryRegionOps slm_mmio_ops = {
    .read = slm_mmio_read,
    .write = slm_mmio_write,
    .endianness = DEVICE_LITTLE_ENDIAN,
    .impl = {
        .min_access_size = 2,
        .max_access_size = 8,
...
static void nvme_ns_init_identify_cs_indep(FemuCtrl *n, NvmeIdNsCsIndep *id_ns)
{
...
static int nvme_init_namespace(FemuCtrl *n, NvmeNamespace *ns, Error **errp)
{
    ...
    nvme_ns_init_identify_cs_indep(n, &ns->id_ns_cs_indep);
    ...
    if (ns->csi < NVME_CSI_COMPUTE) {
        if (ns->size == 0) {
            femu_err("Namespace size is 0\n");
            abort();
        }
...
static int nvme_init_namespaces(FemuCtrl *n, Error **errp)
{
    for (int i = 0; i < n->num_namespaces; i++) {
        ...
        femu_debug("namespace %d: size %zu, type %s\n", ns->id, ns->size, nvme_csi_str(ns->csi));
...
static void nvme_delete_namespace(NvmeNamespace *ns)
{
    if (ns->backend) {
        free_backend(ns->backend);
    }
    if (ns->ops->exit) {
        ns->ops->exit(ns);
    }
...
{
    static atomic_int fsid_vbb = ATOMIC_VAR_INIT(0);
    const char *vbbssd_mn = "FEMU Controller";
    const char *vbbssd_sn = "vSSD";
    int id = atomic_fetch_add(&fsid_vbb, 1);
    n->drive_id = id;

    nvme_set_ctrl_name(n, vbbssd_mn, vbbssd_sn, &id);
}
...
static void nvme_init_ctrl(FemuCtrl *n)
{
    ...
    id->rsvd534      = cpu_to_le16(0x0f);// copy descriptor formats supported
...
static void nvme_init_pci(FemuCtrl *n)
{
    ...
    memory_region_init_io(&n->slm_mem, OBJECT(n), &slm_mmio_ops, n, "nvme-slm",
                          n->namespaces[1].size);
    pci_register_bar(&n->parent_obj, 2, PCI_BASE_ADDRESS_SPACE_MEMORY |
                     PCI_BASE_ADDRESS_MEM_TYPE_64, &n->slm_mem);
...
{
    // TODO: if set dump config file, override all parameters here
    FemuCtrl *n = FEMU(pci_dev);
    if (parse_config(n)) {
        femu_err("parse config file err!");
        abort();
    }
    ...
    if (nvme_check_constraints(n)) {
        femu_err("nvme_check_constraints failed!");
        return;
    }
    ...
    n->sq = g_malloc0(sizeof(NvmeSQueue*) * (n->nr_io_queues + 1));
    n->cq = g_malloc0(sizeof(NvmeCQueue*) * (n->nr_io_queues + 1));
    n->elpes = g_malloc0(sizeof(NvmeErrorLog) * (n->elpe + 1));
    n->aer_reqs = g_malloc0(sizeof(NvmeRequest*) * (n->aerl + 1));
...
static void femu_exit(PCIDevice *pci_dev)
{
    ...
    for (int i = 0; i < n->num_namespaces; i++) {
        nvme_delete_namespace(&n->namespaces[i]);
    }
    nvme_clear_ctrl(n, true);
...
{
    pthread_spin_init(&csd_memory_region_lock, PTHREAD_PROCESS_PRIVATE);
    pthread_spin_init(&stat_lock, PTHREAD_PROCESS_PRIVATE);
