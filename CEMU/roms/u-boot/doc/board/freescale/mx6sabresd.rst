...
.. code-block:: bash
   ...
   $ dd if=SPL of=/dev/sdX bs=1K seek=1 conv=notrunc && sync

- Flash the u-boot-dtb.img binary into the SD card:

.. code-block:: bash

   $ dd if=u-boot-dtb.img of=/dev/sdX bs=1K seek=69 conv=notrunc && sync
...
.. code-block:: bash
   ...
   $ dd if=SPL of=/dev/sdX bs=1K seek=1 oflag=sync status=none conv=notrunc && sync

- Flash the u-boot-dtb.img image into the SD card

.. code-block:: bash

   $ dd if=u-boot-dtb.img of=/dev/sdX bs=1K seek=69 oflag=sync status=none conv=notrunc && sync

Create a partition for root file system and extract it there

.. code-block:: bash

   $ tar xvf rootfs.tar.gz -C /media/root
