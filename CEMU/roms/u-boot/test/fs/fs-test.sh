...
function create_files() {
	...
	if [ ! -f "${GB2p5}" ]; then
		dd if=/dev/urandom of="${GB2p5}" bs=1M count=1 \
			&> /dev/null
		dd if=/dev/urandom of="${GB2p5}" bs=1M count=2 seek=2047 \
			&> /dev/null
		dd if=/dev/urandom of="${GB2p5}" bs=1M count=1 seek=2499 \
			&> /dev/null
	fi

	# Create a small file in this image.
	if [ ! -f "${MB1}" ]; then
		dd if=/dev/urandom of="${MB1}" bs=1M count=1 \
			&> /dev/null
	fi
...
for fs in ext4 fat16 fat32; do
	...
	mount -o loop,rw,$uid "$IMAGE" "$MOUNT_DIR"
	chmod 777 "$MOUNT_DIR"
